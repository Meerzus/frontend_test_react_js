...
### 5. Локализация:
   - **Добавить переключение языка интерфейса (`русский` / `английский` / `итальянский`). Весь контент должен быть переведен на выбранный язык (необязательно, но будет плюсом). Язык тоже должен сохраняться после перезагрузки страницы.**

### 6. Дизайн:
   - **Приложение должно корректно отображаться на `десктопах`, `планшетах` и на `мобильных устройствах`.**

### 7. Необязательное:
   - **Будет плюсом, если исполнитель добавит свой компонент (например с аналитикой данных). Можно использовать сторонние библиотеки.**
...
######
   - #### `UserItem.jsx` — компонент, представляющий отдельного сотрудника, с кнопкой для удаления из списка. ФИО сотрудника должно быть отформатировано под вид `Фамилия И. О.`, `Фамилия И.` или `Имя О.`.
######
